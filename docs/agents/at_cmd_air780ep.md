...
| 查询 PDP 地址 | `AT+CGPADDR=<cid>` | `+CGPADDR: <cid>,<PDP_ipv4_addr>[,<PDP_ipv6_addr>]` + `OK` | IPv4/IPv6 地址字符串 | 9s | `modem_get_pdp_context()` 组合/缓存 `AT+CGDCONT?`、`AT+CGACT?` 和 `AT+CGPADDR` 的结果 | `AT+CGPADDR` 只提供地址，不能单独还原 APN/激活状态；PDP 未建立时无法查询地址 |
...
| TCPIP 设置 APN | `AT+CSTT` 或 `AT+CSTT=<apn>[,<username>,<password>]` | `OK` | APN 为空时模块按自动获取 APN 设置 | 60s | `modem_activate_pdp()` 的 Air780EP TCPIP 激活步骤 | 仅在 `IP INITIAL` 状态设置有效，成功后进入 `IP START` |
| 激活移动场景 | `AT+CIICR` | `OK` 或 `ERROR` | 激活 GPRS/PDP 场景 | 90s | `modem_activate_pdp()` 的 Air780EP TCPIP 激活步骤 | 只在 `IP START` 状态有效，成功后进入 `IP GPRSACT` |
| 查询本地 IP | `AT+CIFSR` | `<IP address>` 或 `ERROR` | 本地 IP 字符串 | 9s | `modem_activate_pdp()` 成功后填充 `modem_pdp_context_t.ip_addr` | 仅在场景已激活后可用；返回纯 IP 成功行，不是 `OK` 终止的常规响应，也不是 `+CIFSR:` 前缀；当前 `at_engine` 需自定义 final 处理或 wrapper 支持后才能按字面使用 |
...
| 关闭移动场景 | `AT+CIPSHUT` | `SHUT OK` 或 `ERROR` | 无 | 90s | `modem_deactivate_pdp()` 或错误恢复清理步骤 | 多连接时会关闭所有 IP 连接；`SHUT OK` 不是当前 `at_engine` 标准 final 行，需要自定义 final 处理或扩展 AT Engine |
